...
function HospitalList({
  ...
  searchPlaceholder,
  noResultsMessage,
}: Omit<ComboboxProps, 'className' | 'placeholder' | 'value' | 'items'> & {
  items: { value: string; label: string }[];
  value: string;
  onChange: (value: string) => void;
  onSelect: () => void;
          ...
          {items.map((item) => (
            <CommandItem
              ...
              onSelect={(currentValue) => {
                const newValue = currentValue.toLowerCase() === value.toLowerCase() ? "" : item.value
                onChange(newValue)
                onSelect()
              }}
            >
        ...
        <Drawer open={open} onOpenChange={setOpen}>
            <DrawerTrigger asChild>
                <Button variant="outline" className={cn("w-full justify-start", className)}>
                    {selectedLabel ? <span className="truncate">{selectedLabel}</span> : placeholder}
